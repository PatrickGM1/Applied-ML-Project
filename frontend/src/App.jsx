...
function getApiBaseUrl() {
  ...
  // Vite proxy handles /predict → localhost:8000 in dev
  // In production (served from FastAPI), relative URLs work too
  return ''
}
...
export default function App() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.statement.trim()) {
      setResult({ error: 'Claim text is required.' })
      return
    }
    ...
    const payload = {
      ...
      hist1: Number(form.hist1 || 0),
      hist2: Number(form.hist2 || 0),
      hist3: Number(form.hist3 || 0),
      hist4: Number(form.hist4 || 0),
    ...
    try {
      ...
      setResult({
        error: `Could not reach API at ${API_BASE_URL || 'localhost:8000'}. Make sure the backend is running.`,
      })
    } finally {
      setLoading(false)
    }
    ...
    <div className="container">
      <header className="hero">
        <div className="hero-domain">
          <a href="/">
            aml<span className="hero-slash">.</span>guba
            <span className="hero-slash">.</span>dev
          </a>
        </div>
        <p>Fake News Detection — Base Model</p>
      </header>
      ...
      <section>
        ...
        <form className="predict-form" onSubmit={handleSubmit}>
          ...
          <div className="grid-2">
            ...
            <div>
              <label htmlFor="speaker_job">Speaker job</label>
              <input
                id="speaker_job"
                name="speaker_job"
                type="text"
                placeholder="senator"
                value={form.speaker_job}
                onChange={handleChange}
              />
            </div>
          </div>
      ...
      <footer>
        © 2026 <a href="https://guba.dev">guba.dev</a>
      </footer>
    </div>
  )
}
...
function PredictResult({ result, loading }) {
  if (loading) {
    return (
      <div className="result result--loading">
        ...
        <p className="muted">Running inference…</p>
      </div>
    )
  }
  ...
  if (!result) {
    return (
      <div className="result">
        <p className="muted">Submit a claim to see model output.</p>
      </div>
    )
  }
  ...
  const { data } = result
  const probabilities = Object.entries(data.class_probabilities || {}).sort(
    ([, a], [, b]) => b - a,
  )
    ...
    <div className="result result--success">
      <div className="result-header">
        <div className="result-meta">
          <p className="result-predicted-label">
            <span className="result-verdict">{data.predicted_label}</span>
          </p>
          {confidencePct && (
            <p className="muted result-confidence-text">
              Confidence:{' '}
...
// Circular confidence meter using SVG arc
// r=15.9 → circumference ≈ 99.9, so strokeDasharray="${pct} 100" works directly
