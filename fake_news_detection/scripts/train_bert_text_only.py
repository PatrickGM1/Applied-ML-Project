"""Train and evaluate a BERT-style text-only classifier on LIAR.

This implements "Model 1: BERT (text only)" from Chat_Idea.md.

It mirrors the existing TF-IDF baselines:
- Train on train split
- Evaluate on valid split
- Save metrics as JSON + a short TXT summary

Run (from repo root):
  python -m fake_news_detection.scripts.train_bert_text_only

Tips:
  - For a quick smoke test use --max-train-samples/--max-eval-samples.
  - By default this does NOT save model checkpoints (keeps repo small).
"""
...
OUTPUT_DIR = PROJECT_DIR / "artifacts" / "bert"

TEXT_COLUMN_PREFERRED = "statement"  # use raw statement when available
...
def truncate_frame(frame: pd.DataFrame, limit: Optional[int], seed: int) -> pd.DataFrame:
    if not limit or limit <= 0 or limit >= len(frame):
        return frame
    # Shuffle first so we don't take a biased prefix.
...
def save_results(name: str, metrics: dict, train_rows: int, valid_rows: int, label_column: str, model_name: str) -> None:
...
def run_experiment(
    ...
    num_labels = int(pd.concat([train_df[label_column], valid_df[label_column]]).nunique())
    ...
    valid_labels = valid_df[label_column].astype(int).tolist()

    train_dataset = TextDataset(train_texts, train_labels, tokenizer=tokenizer, max_length=max_length)
    valid_dataset = TextDataset(valid_texts, valid_labels, tokenizer=tokenizer, max_length=max_length)
    ...
    trainer.train()
    # Get predictions for full report/confusion matrix.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BERT text-only models on LIAR (train→valid).")
    ...
    parser.add_argument("--max-train-samples", type=int, default=0, help="Optional limit for quick runs")
    parser.add_argument("--max-eval-samples", type=int, default=0, help="Optional limit for quick runs")
...
def main() -> None:
    ...
    for name, config in EXPERIMENTS.items():
        ...
        valid_df = truncate_frame(valid_df, args.max_eval_samples, seed=args.seed)
